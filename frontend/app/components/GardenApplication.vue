...
type AppView = 'today' | 'garden' | 'journal' | 'harvests' | 'settings'
const routeView = computed<AppView>(() => {
  const value = typeof route.params.view === 'string' ? route.params.view : 'today'
  return ['today', 'garden', 'journal', 'harvests', 'settings'].includes(value)
    ? value as AppView
    : 'today'
})
...
<template>
  ...
  <div v-else class="app-shell">
    <GardenSidebar
      :active-view="routeView"
      :user-name="user.name"
    />

    <main class="workspace">
      <header v-if="routeView !== 'settings'" class="topbar">
        <div>
      ...
      <section v-else-if="!gardens.length && routeView !== 'settings'" class="empty-hero">
